...
class Instagram
{
    ...
    public static function getMedias($username, $count = 20, $maxId = '')
    {
        ...
        while ($index < $count && $isMoreAvailable) {
            ...
            if ($response->code !== 200) {
                throw new InstagramException('Response code is '.$response->code.'. Something went wrong. Please report issue.');
            }
    ...
    public static function getMediasByURL($url, $count = 20, $type, $maxId = '')
    {
        $index = 0;
        $medias = [];
        $isMoreAvailable = true;
        while ($index < $count && $isMoreAvailable) {
            $response = Request::get(Endpoints::getAccountMediasViaLink($url, $maxId));
            if ($response->code !== 200) {
                throw new InstagramException('Response code is '.$response->code.'. Something went wrong. Please report issue.');
            }
            ...
            if($type == "tag"){
                ...
                if (count($arr['location']['media']['count']) === 0) {
                    return [];
                }
                ...
                $maxId = $arr['location']['media']['page_info']['end_cursor'];
                $hasNextPage = $arr['location']['media']['page_info']['has_next_page'];
            }else{
                if (count($arr['items']) === 0) {
                    return [];
                }
