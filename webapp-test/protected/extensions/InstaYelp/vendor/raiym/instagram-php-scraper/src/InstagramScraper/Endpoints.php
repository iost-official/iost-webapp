...
class Endpoints
{
    ...
    public static function getAccountMediasViaLink($url, $maxId = '')
    {
        $url = trim($url);
        if(strpos($url, "/tags/") !== false || strpos($url, "/locations/") !== false){ 
            $url .= "?__a=1&max_id={max_id}";
        }else{
            //normal account URL
            $url .= "media?max_id={max_id}";
        }
